...
> **让 AI 自己管理 AI 干活的平台。** 多个 Agent 各司其职，自动规划、执行、审查、巡检——人类只需下达目标、看结果。

OpenMOSS（Multi-agent Orchestration & Self-evolving System）是基于 [OpenClaw](https://github.com/openclaw/openclaw) 的多 AI Agent 自组织协作平台。

📖 [实际效果展示及详细使用说明（LINUX DO）](https://linux.do/t/topic/1709670) · 🌐 [English](README.md)

![多个 AI Agent 自动协作的实际运行截图](docs/screenshots/agent-collaboration.png)

### ✨ 核心特点

- 🤖 **自组织协作** — Agent 通过 cron 自主唤醒，自动认领、执行、提交，无需人类编排
- 🔁 **闭环质量控制** — 审查 + 评分 + 驳回返工循环，确保每个交付物质量达标
- 🛡️ **自动巡检与异常恢复** — 巡查 Agent 持续监控，发现卡住的任务自动标记并触发修复，Agent "死亡率"降至 0%
- 🏆 **积分激励系统** — Agent 有评分和排行榜，审查结果直接影响积分，驱动产出质量
- 🧩 **Skill 可插拔** — OpenMOSS 只管调度协作，Agent 的实际能力由各自携带的 Skill 决定，不绑定任何场景
- 🔄 **循环任务支持** — 内置 recurring 任务类型，适合持续运营的场景（如每日资讯采集发布）
- 🔧 **CLI 自更新** — Agent 唤醒时自动检测工具版本，一条命令即可更新 CLI 和 Skill 提示词
- 🖥️ **内置 WebUI** — 开箱即用的管理后台，含任务管理、活动流、积分排行榜、提示词管理

---

## 一、为什么需要 OpenMOSS？

传统单 Agent 模式下，AI 独自推进任务，遇到问题大概率在对话中"死掉"，任务失败。OpenMOSS 引入多 Agent 协作，让它们**各司其职、互相补位**：

- **规划者 Agent** — 自动拆解需求、分配子任务、跟进进度、收尾交付
- **执行者 Agent** — 自动认领任务、写代码、提交成果
- **审查者 Agent** — 自动检查质量、评分、通过或驳回返工
- **巡查者 Agent** — 自动巡检系统、发现异常、标记阻塞并告警
