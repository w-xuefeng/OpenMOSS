...
const ROUTE_RULES: RouteRule[] = [
  {
    pattern: /POST \/api\/agents\/register/,
    icon: 'UserPlus',
    verb: '注册了新 Agent',
    colorClass: 'bg-emerald-500',
    extractFields: ['name', 'role'],
  },
  {
    pattern: /POST \/api\/tasks$/,
    icon: 'ListPlus',
    verb: '创建了任务',
  ...
  {
    pattern: /POST \/api\/tasks\/.*\/modules/,
    icon: 'FolderPlus',
    verb: '创建了模块',
  ...
  {
    pattern: /POST \/api\/sub-tasks\/.*\/claim/,
    icon: 'Hand',
    verb: '认领了子任务',
    colorClass: 'bg-amber-500',
  },
  {
    pattern: /POST \/api\/sub-tasks\/.*\/start/,
    icon: 'Play',
    verb: '开始执行子任务',
    colorClass: 'bg-orange-500',
  },
  {
    pattern: /POST \/api\/sub-tasks\/.*\/submit/,
    icon: 'PackageCheck',
    verb: '提交了子任务',
    colorClass: 'bg-green-500',
  },
  {
    pattern: /PUT \/api\/sub-tasks\//,
    icon: 'Pencil',
    verb: '编辑了子任务',
    colorClass: 'bg-slate-500',
  },
  {
    pattern: /POST \/api\/review-records/,
    icon: 'FileSearch',
    verb: '提交了审查',
    colorClass: 'bg-purple-500',
    extractFields: ['score', 'comment', 'result'],
  },
  {
    pattern: /POST \/api\/logs/,
    icon: 'MessageSquare',
    verb: '写了活动日志',
    colorClass: 'bg-gray-500',
    extractFields: ['action', 'summary'],
  },
  {
    ...
    extractFields: ['score_delta', 'reason'],
  },
  {
    pattern: /GET \/api\/rules/,
    icon: 'BookOpen',
    verb: '查询了规则指令',
    colorClass: 'bg-sky-500',
    extractFields: ['task_id', 'sub_task_id'],
  },
  {
  ...
  {
    pattern: /GET \/api\/sub-tasks\/available/,
    icon: 'Search',
    verb: '查找可认领子任务',
    colorClass: 'bg-cyan-500',
    extractFields: ['status', 'priority', 'type'],
  },
  {
    pattern: /GET \/api\/sub-tasks\/mine/,
    icon: 'ClipboardList',
    verb: '查看了自己的子任务',
    colorClass: 'bg-slate-400',
  ...
  {
    pattern: /GET \/api\/sub-tasks\/latest/,
    icon: 'FileText',
    verb: '查看最新子任务',
    colorClass: 'bg-slate-400',
  },
  {
    pattern: /GET \/api\/sub-tasks/,
    icon: 'ClipboardList',
    verb: '查看了子任务',
    colorClass: 'bg-slate-400',
    extractFields: ['status', 'assigned_agent'],
  },
  {
    pattern: /GET \/api\/scores\/me/,
    icon: 'Award',
    verb: '查看了自己的积分',
    colorClass: 'bg-amber-400',
  },
  {
    pattern: /GET \/api\/scores\/leaderboard/,
    icon: 'Medal',
    verb: '查看了排行榜',
    colorClass: 'bg-yellow-500',
  },
  {
    pattern: /GET \/api\/config\/notification/,
    icon: 'Bell',
    verb: '查询了通知配置',
    colorClass: 'bg-rose-500',
  },
  {
    pattern: /GET \/api\/logs\/mine/,
    icon: 'ScrollText',
    verb: '查看了自己的日志',
  ...
  },
  {
    ...
    verb: '查看了活动日志',
    colorClass: 'bg-gray-400',
    extractFields: ['action', 'agent_id'],
  },
  {
    pattern: /GET \/api\/review-records/,
    icon: 'FileSearch',
    verb: '查看了审查记录',
    colorClass: 'bg-purple-400',
    extractFields: ['sub_task_id'],
  },
];
...
export function getActionCategory(action: RecentAction): ActionCategory {
  ...
  // 完成类
  if (/POST .*\/submit/.test(key) || /POST .*\/review-records/.test(key)) {
    return 'complete';
  }

  // 执行类
  if (
    /POST .*\/claim/.test(key) || /POST .*\/start/.test(key) ||
    /PUT \/api\/sub-tasks/.test(key)
  ) return 'execute';
...
export function getActionVerb(action: RecentAction): string {
  ...
  const rule = ROUTE_RULES.find((r) => r.pattern.test(key));
  return rule?.verb ??
    `${action.method} ${action.path.split('/api')[1] ?? action.path}`;
}
